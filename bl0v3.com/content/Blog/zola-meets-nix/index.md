...
up generating these files you are viewing involves other aspects to take care of as well.
...
While so far everything I have done is obtaining zolas source code and writing 
my blogposts into the respective markdown files. There is a little more to it as:

### What if I wanted to edit my blog from multiple machines and track changes?

While i could simply put my blog into a git repository like I've done [here](https://github.com/bolives-hax/bl0v3.com-static/tree/master) (Contains what you are currently viewing) there are things I wanted to take into account that just doing this
wouldn't solve.
...
want to make use tools other than zola to create pages or content found on these pages? Like maybe I'd like to
...
while i specify `name` the `?` operator can be used to assign default values if `name` wasn't specified.
...
`nixpkgs` provides the derivations/package definition for all packages contained within nixpkgs 
_(which for example is where nixos gets its packages from. But due to nix's nature it works on a mac or non-nixos linux host just as well :3 )_. `duckquill-theme` is the theme i use and `website-src`  provides the actual source of the markdown files and
co referenced by zola. And `flake-utils` provides some helpers (more on that later).

Thus all that is left to do is combine the flakes inputs and declarations to form the output derivations:
...
but I may tough upon that in another post in the future)_
...
```
etc etc manually but I like to keep it short, reduce code duplication and have a somewhat future proof setup as maybe
in the future another architecture gets adopted as a default. Thus `eachDefaultSystem` essentially just generates
these struct declaration prefixes and calls the function following `(system: ` which was written by me and
essentially generates the derivation containing the actual build instructions for the blog using various urls.

Note that `system:` is passed as a parameter to the function and contains the current system the attributes are emitted
for. So the function within would essentially be called once with `x86_64-linux` once with `x86_64-darwin` and so on.
...
do too much accounting for the shortcomings listed above when introducing more components into the website build process
...
to a module contained within **/etc/nixos**.  Of course id need to provide the
`bl0v3_website` definition reference in
```nix
{
    root = "${bl0v3_website.packages.${system}.bl0v3_dot_com}";
}
...
systems. _(there is very valid reasons one would not want to use flakes but by the time these start to
be a concern for you you most likely know yourself. Otherwise I'd say its fine to use flakes its just a matter of taste)_
```nix
{
  inputs = {
    bl0v3_static = {
...
while a little hacky it gets the job done for now but i plan to rather work using my CI infrastructure  but for 
now this gets the job done as all id have to do to populate/update that site is run:
